...
def test_fit_single_accepts_direct_file_input(
...
def test_fit_single_accepts_folder_input_when_one_match_exists(
    tmp_path, project_root, write_bruker_esr_sample
) -> None:
...
def test_fit_single_errors_when_folder_resolves_multiple_matches(
    tmp_path, project_root, write_bruker_esr_sample
) -> None:
    source_dir = tmp_path / "raw"
    source_dir.mkdir()
    ...
    with pytest.raises(
        SystemExit, match=r"fit-single requires exactly one discovered \.dsc file, found 2"
    ):
        main(
            [
                "fit-single",
...
def test_fit_batch_can_export_batch_resonance_metrics(
    tmp_path, project_root, write_bruker_esr_sample
) -> None:
    source_dir = tmp_path / "raw"
    source_dir.mkdir()
...
def test_fit_batch_filters_folder_input_with_pattern(
    tmp_path, project_root, write_bruker_esr_sample
) -> None:
    source_dir = tmp_path / "raw"
    source_dir.mkdir()
    ...
    assert exit_code == 0
    rows = list(
        csv.DictReader((output_dir / "batch_summary.csv").open("r", encoding="utf-8", newline=""))
    )
    assert [row["source_stem"] for row in rows] == ["alpha_deg_trace"]


def test_fit_batch_recursively_discovers_nested_files(
    tmp_path, project_root, write_bruker_esr_sample
) -> None:
    source_dir = tmp_path / "raw"
    ...
    assert [item["source_file"] for item in payload["items"]] == [
        str((nested_dir / "nested-0deg-R1.dsc").resolve())
    ]


def test_esr_batch_prints_batch_overlay_path(
    tmp_path, project_root, write_bruker_esr_sample, capsys
) -> None:
    source_dir = tmp_path / "raw"
    source_dir.mkdir()
    write_bruker_esr_sample(source_dir / "sample-0deg-R1.dsc")
    ...
    assert (
        f"Batch figure [batch_processed_offset_R1]: {output_dir / 'batch_processed_offset_R1.png'}"
        in output
    )
    assert (
        f"Batch figure [batch_angle_overlay_R1]: {output_dir / 'batch_angle_overlay_R1.png'}"
        in output
    )
    assert (
        f"Batch figure [batch_processed_offset_R2]: {output_dir / 'batch_processed_offset_R2.png'}"
        in output
    )
    assert (
        f"Batch figure [batch_angle_overlay_R2]: {output_dir / 'batch_angle_overlay_R2.png'}"
        in output
    )
