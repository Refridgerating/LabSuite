...
def _build_trace(*, double: bool = False) -> tuple[FmrTraceDataset, FmrProcessedTrace]:
    ...
    if double:
        signal += mixed_derivative_lorentzian(
            ...
            H_res_mT=118.0,
            DeltaH_mT=8.0,
            amplitude_symmetric=18.0,
            amplitude_antisymmetric=4.0,
            baseline_offset=0.0,
            baseline_slope=0.0,
        )
...
def test_detect_candidate_windows_finds_two_features_for_double_trace() -> None:
    ...
    windows = detect_candidate_windows(
        processed_trace.field_mT, processed_trace.signal, FmrRecipe()
    )
...
def test_fit_fmr_trace_selects_double_mode_for_two_resonance_trace() -> None:
    ...
    assert result.r_squared is not None
    assert [component.component_label for component in result.selected_components] == [
        "mode_1",
        "mode_2",
    ]
...
def test_auto_mode_stays_single_when_double_gain_is_below_threshold() -> None:
    raw_trace, processed_trace = _build_trace(double=True)
    recipe = FmrRecipe(double_fit_min_improvement_ratio=1.0)
    result = fit_fmr_trace(raw_trace, processed_trace, recipe)
    assert result.double_fit_improvement_ratio is not None
    assert result.double_fit_improvement_ratio < recipe.double_fit_min_improvement_ratio
    assert result.selected_mode == "single"
